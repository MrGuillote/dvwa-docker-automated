...
echo -e "${COLOR_VERDE}Deteniendo el servidor Apache en caso de problemas...${COLOR_RESET}"
...
if ! check_installed docker; then
    echo -e "${COLOR_VERDE}Instalando Docker...${COLOR_RESET}"
    sudo apt update
...
if ! systemctl is-active --quiet docker; then
    echo -e "${COLOR_VERDE}Iniciando Docker...${COLOR_RESET}"
...
echo -e "${COLOR_VERDE}Descargando la imagen DVWA...${COLOR_RESET}"
...
if sudo docker ps --format '{{.Names}}' | grep -q "dvwa_container"; then
    ...
    echo -e "${COLOR_VERDE}Ejecutando la máquina virtual DVWA...${COLOR_RESET}"
...
echo ""
echo -e "${COLOR_VERDE}http://127.0.0.1:4200${COLOR_RESET}"
echo ""
echo -e "${COLOR_VERDE}Usuario: admin${COLOR_RESET}"
echo -e "${COLOR_VERDE}Contraseña: password${COLOR_RESET}"
echo ""
echo -e "${COLOR_VERDE}Recuerda hacer clic en el botón 'Create / Reset Database'.${COLOR_RESET}"
echo ""

# Mostrar mensaje para detener el contenedor manualmente si es necesario
echo -e "${COLOR_VERDE}Si necesitas detener el contenedor DVWA, ejecuta: sudo docker stop dvwa_container${COLOR_RESET}"
